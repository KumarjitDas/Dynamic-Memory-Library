...
START_TEST(null_test) {
   ...
   ck_assert_int_eq(DynMemCopyInitial(NULL, NULL), DYNMEM_FAILED);
   ck_assert_int_eq(DynMemAllocate(&dynmem, 4, 5, NULL), DYNMEM_SUCCEED);
   ck_assert_int_eq(DynMemCopyInitial(NULL, &dynmem), DYNMEM_FAILED);
   ck_assert_int_eq(DynMemCopyInitial(&dynmem, NULL), DYNMEM_FAILED);
...
START_TEST(value_not_added) {
   ...
   ck_assert_int_eq(DynMemCopyInitial(&destination, &source), DYNMEM_SUCCEED);
...
int main() {
   Suite *suite = suite_create("Test suite for \"DynMemCopyInitial\" function");
