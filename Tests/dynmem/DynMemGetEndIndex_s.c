...
START_TEST(no_value_added) {
   ...
   ck_assert_int_eq(DynMemGetEndIndex_s(NULL, NULL), DYNMEM_FAILED);
   ck_assert_int_eq(DynMemGetEndIndex_s(NULL, &index), DYNMEM_FAILED);
   ck_assert_int_eq(index, -1);
   ck_assert_int_eq(DynMemAllocate(&dynmem, 4, 5, NULL), DYNMEM_SUCCEED);
   ck_assert_int_eq(DynMemGetEndIndex_s(&dynmem, NULL), DYNMEM_FAILED);
   ck_assert_int_eq(DynMemGetEndIndex_s(&dynmem, &index), DYNMEM_SUCCEED);
   ck_assert_int_eq(index, -1);
...
START_TEST(appended) {
   ...
   ck_assert_int_eq(DynMemGetEndIndex_s(&dynmem, &index), DYNMEM_SUCCEED);
   ck_assert_int_eq(index, 16);

   for (int i = 5; i < 10; i++)
      ck_assert_int_eq(DynMemAppend(&dynmem, &i), DYNMEM_SUCCEED);

   ck_assert_int_eq(DynMemGetEndIndex_s(&dynmem, &index), DYNMEM_SUCCEED);
   ck_assert_int_eq(index, 36);

   for (int i = 10; i < 20; i++)
      ck_assert_int_eq(DynMemAppend(&dynmem, &i), DYNMEM_SUCCEED);

   ck_assert_int_eq(DynMemGetEndIndex_s(&dynmem, &index), DYNMEM_SUCCEED);
   ck_assert_int_eq(index, 76);
...
int main() {
   Suite *suite = suite_create("Test suite for \"DynMemGetEndIndex_s\" function");
